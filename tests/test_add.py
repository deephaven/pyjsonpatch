...
class AddInDict(BaseTest):
    ...
    def test_add_a_composite(self):
        self.assertPatch({"foo": 1}, [add("/bar", [1, 2])], {"foo": 1, "bar": [1, 2]})
...
class AddInList(BaseTest):
    ...
    def test_size_2_append(self):
        self.assertPatch(
            [None, "foo"], [add("/-", "bar")], [None, "foo", "bar"], strict_patch=False
        )

    def test_size_2_insert_0(self):
        self.assertPatch(
            [None, "foo"], [add("/0", "bar")], ["bar", None, "foo"], strict_patch=False
        )

    def test_size_2_insert_1(self):
        self.assertPatch(
            [None, "foo"], [add("/1", "bar")], [None, "bar", "foo"], strict_patch=False
        )
...
class AddReplacesInDict(BaseTest):
    def test_value_to_none(self):
        self.assertPatch({"foo": 1}, [add("/foo", None)], {"foo": None}, strict_patch=False)

    def test_none_to_value(self):
        self.assertPatch({"foo": None}, [add("/foo", 1)], {"foo": 1}, strict_patch=False)

    def test_true_to_false(self):
        self.assertPatch({"foo": True}, [add("/foo", False)], {"foo": False}, strict_patch=False)

    def test_false_to_true(self):
        self.assertPatch({"foo": False}, [add("/foo", True)], {"foo": True}, strict_patch=False)
    ...
    def test_dict_to_list(self):
        self.assertPatch({"foo": {"bar": 1}}, [add("/foo", [1])], {"foo": [1]}, strict_patch=False)
