...
class CopyReplacesRoot(BaseTest):
    def test_dict_to_dict(self):
        self.assertPatch(
            {"child": {"foo": 1}}, [copy("/child", "")], {"foo": 1}, strict_patch=False
        )

    def test_dict_to_list(self):
        self.assertPatch({"child": [1]}, [copy("/child", "")], [1], strict_patch=False)

    def test_list_to_dict(self):
        self.assertPatch(["hello", {"foo": 1}], [copy("/1", "")], {"foo": 1}, strict_patch=False)

    def test_list_to_list(self):
        self.assertPatch(["hello", [1]], [copy("/1", "")], [1], strict_patch=False)
