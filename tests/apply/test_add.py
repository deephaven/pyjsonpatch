...
class AddInList(BaseTest):
    def test_size_0_append(self):
        self.assertPatch([], [add("/-", "foo")], ["foo"])

    def test_size_0_insert_0(self):
        self.assertPatch([], [add("/0", "foo")], ["foo"])

    def test_size_2_append(self):
        self.assertPatch([None, "foo"], [add("/-", "bar")], [None, "foo", "bar"])

    def test_size_2_insert_0(self):
        self.assertPatch([None, "foo"], [add("/0", "bar")], ["bar", None, "foo"])

    def test_size_2_insert_1(self):
        self.assertPatch([None, "foo"], [add("/1", "bar")],[None, "bar", "foo"])

    def test_size_2_insert_2(self):
        self.assertPatch([None, "foo"], [add("/2", "bar")], [None, "foo", "bar"])
...
class AddReplacesInDict(BaseTest):
    def test_value_to_none(self):
        self.assertPatch({"foo": 1}, [add("/foo", None)], {"foo": None})

    def test_none_to_value(self):
        self.assertPatch({"foo": None}, [add("/foo", 1)], {"foo": 1})

    def test_true_to_false(self):
        self.assertPatch({"foo": True}, [add("/foo", False)], {"foo": False})

    def test_false_to_true(self):
        self.assertPatch({"foo": False}, [add("/foo", True)], {"foo": True})

    def test_list_to_dict(self):
        self.assertPatch({"foo": [1]}, [add("/foo", {"bar": 1})], {"foo": {"bar": 1}})

    def test_dict_to_list(self):
        self.assertPatch({"foo": {"bar": 1}}, [add("/foo", [1])], {"foo": [1]})


class AddReplacesInRoot(BaseTest):
    def test_value_to_none(self):
        self.assertPatch(1, [add("", None)], None)

    def test_none_to_value(self):
        self.assertPatch(None, [add("", 1)], 1)

    def test_true_to_false(self):
        self.assertPatch(True, [add("", False)], False)

    def test_false_to_true(self):
        self.assertPatch(False, [add("", True)], True)

    def test_list_to_dict(self):
        self.assertPatch([1], [add("", {"foo": 1})], {"foo": 1})

    def test_dict_to_list(self):
        self.assertPatch({"foo": 1}, [add("", [1])],[1])
